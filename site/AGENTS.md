...
- 开发命令：`cd site && npm run dev` → `http://localhost:4321`
- 构建命令：`cd site && npm run build` → 产出 `site/dist/`
- 本地预览构建产物：`cd site && npm run preview`
- 部署：`cd site && vercel --prod`（首次需 `vercel` 链接项目，再到 Vercel 后台绑定 `quotabar.ddonlien.com` 域名）

### 文档入口

- 子项目说明：`site/README.md`
- 视觉规范：`site/DESIGN.md`
- 需求追踪：`site/REQUIREMENTS.md`
- 执行日志：`site/agent-log/`
- 全局父级文档：`../AGENTS.md`、`../README.md`、`../REQUIREMENTS.md`、`../DESIGN.md`

### 目录索引

- `site/src/pages/index.astro`：主页入口，组装所有 section。
- `site/src/layouts/Layout.astro`：HTML 骨架 + meta + 滚动入场动画脚本。
- `site/src/styles/global.css`：design tokens（颜色 / 字体 / 间距 / 圆角 / 阴影 / 动效）。
- `site/src/components/`：UI 组件
...
- `site/public/favicon.svg`：站点图标（三段进度条 logo）。
- `site/astro.config.mjs`：Astro 配置（静态输出、site URL）。

### 项目特殊约束

- **语言与命名**：面向用户的文案默认中文；HTML `lang="zh-CN"`；CSS 类名沿用英文 kebab-case。
- **视觉一致性**：颜色 token 必须与 macOS 应用 `QuotaModels.swift` 的 `brandColor` 保持一致（详见 `site/DESIGN.md`）。新增 provider 颜色时同步更新两边。
...
- **不动 Swift 代码**：本子项目只做营销主页，macOS 应用改动在 `../macos/`。
