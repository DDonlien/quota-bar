...
- 对于使用 Git worktree 的项目，推荐采用“主仓库只保存 Git 元数据，实际开发在 worktree 目录中进行”的目录结构：
...
- `worktrees/` 下的每个子目录对应一个实际工作区。目录名应由用户或项目约定决定；当项目没有更具体规则时，推荐与 branch 名称保持一致。
...
- 如果示例或对话中出现 `main`、`feature-ui`、`fix-save`、`ai-experiment` 等名称，它们仅用于说明 worktree 目录层级，不表示 Agent 应按这些名称创建或命名 worktree。
...
- `agent-log/`：根项目执行日志。
- `worktrees/`：Git worktree 工作目录，按新模板规则使用无点号目录名；目录本身被 `.gitignore` 忽略。
