...
final class FetchPipeline {
    ...
    func run(timeout: TimeInterval) async -> ProviderSnapshot {
        switch runMode {
        case .sequential:
            return await runSequential(timeout: timeout)
        case .parallel:
            return await runParallel(timeout: timeout)
        }
    }

    private func runSequential(timeout: TimeInterval) async -> ProviderSnapshot {
        ...
        for strategy in strategies {
            do {
                ...
                lastError = error
                // missingCredentials 中断：用户必须先登录
                if case .missingCredentials = error { break }
            } catch {
                lastErrors[strategy.id] = .transient(detail: error.localizedDescription)
                lastError = .transient(detail: error.localizedDescription)
            }
        }
        return fallbackSnapshot(error: lastError)
    }
...
final class PipelineQuotaProvider: QuotaProvider, @unchecked Sendable {
    ...
    func fetchSnapshot(timeout: TimeInterval) async throws -> ProviderSnapshot {
        await pipeline.run(timeout: timeout)
    }
}