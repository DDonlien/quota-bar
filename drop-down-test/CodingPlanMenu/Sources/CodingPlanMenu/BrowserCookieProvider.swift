    ...
    func fetchSnapshot(timeout: TimeInterval) async throws -> ProviderSnapshot {
        ...
        do {
            ...
            case .privacyAccessDenied:
                throw QuotaFetchError.sourceUnavailable(detail: "需要 Full Disk Access")
        ...
        guard let endpoint else {
            // 浏览器已登录、但本端尚未对接该 provider 的 dashboard：
            // 返回「已登录、等待首次解析」的占位，不报错。
            return ProviderSnapshot(
                kind: kind,
                availability: .available,
                quotas: [
                    QuotaWindow(title: "5小时额度", remainingFraction: 1.0, refreshDescription: "等待 dashboard 接入"),
                    QuotaWindow(title: "周额度", remainingFraction: 1.0, refreshDescription: "等待 dashboard 接入")
                ],
        ...
        // 解析失败：保守返回「已登录、等刷新」，不要把状态打成 fetchFailed。
        return ProviderSnapshot(
            kind: kind,
            availability: .available,
            quotas: [
                QuotaWindow(title: "5小时额度", remainingFraction: 1.0, refreshDescription: "等待下次刷新"),
                QuotaWindow(title: "周额度", remainingFraction: 1.0, refreshDescription: "等待下次刷新")
            ],
            monthlyPrice: kind.fallbackMonthlyPrice,
            fetchedAt: fetchedAt
        )
