...
- **真实额度读取**（P0）：
  ...
  - Kimi 优先读取 Kimi Desktop token-store 调 membership API，拿 Work + Code 额度；CLI OAuth 仅作为 Code-only fallback
  - GLM / Z Code 读取 `~/.zcode/v2/config.json` / `credentials.json`，调用 Z.ai / BigModel plan quota endpoint；quota API 不可用时读取 `coding-plan-cache.json` 展示真实 plan 状态但不伪造额度
...
| 可行方案 | Codex | Claude | Kimi | MiniMax | Antigravity | GLM / Z Code |
|---|---|---|---|---|---|---|
| 配置文件 → API | P2：`auth.json` access token → `https://chatgpt.com/backend-api/wham/usage` | P1：Claude OAuth → Anthropic usage 或 Messages headers | P1：Kimi Desktop token-store access token → `www.kimi.com/...GetSubscriptionStat`；P2：CLI OAuth → Code-only `https://api.kimi.com/coding/v1/usages` | P1：`~/.mmx/config.json` / `~/.mavis/config.yaml` API key → `https://api.minimaxi.com/v1/coding_plan/remains` | 跳过：不优先 | P1：`~/.zcode/v2/config.json` 读启用 plan 的 API key/baseURL 后调 plan usage；P2：`coding-plan-cache.json` 只读 plan 状态，不生成额度 |
...
| HTTP API 指令 | P2：`wham/usage`，解析 primary/secondary 或 headers | P1：`api.anthropic.com` rate-limit headers | P1：Kimi Desktop token 调 Web membership API 补完整额度；P2：Kimi coding usage | P1：MiniMax coding plan remains | P1：本地 endpoint 包装成 HTTP/gRPC-Web | P1：Z Code anthropic-compatible plan endpoint |
