...
| 配置/凭证文件 | P1：`~/.codex/auth.json` | P1：`~/.claude/.credentials.json` | P2：`~/.kimi-code/credentials/kimi-code.json` | P2：`~/.mmx/config.json`、`~/.mavis/config.yaml` | 待验证：本地配置路径未定 | P2：`~/.zcode/v2/config.json`、`~/.zcode/v2/credentials.json` |
...
| 可行方案 | Codex | Claude | Kimi | MiniMax | Antigravity | GLM / Z Code |
|---|---|---|---|---|---|---|
| 配置文件 → API | P2：`auth.json` access token → `https://chatgpt.com/backend-api/wham/usage` | P1：Claude OAuth → Anthropic usage 或 Messages headers | P2：OAuth → `https://api.kimi.com/coding/v1/usages` | P1：`~/.mmx/config.json` / `~/.mavis/config.yaml` API key → `https://api.minimaxi.com/v1/coding_plan/remains` | 跳过：不优先 | P1：`~/.zcode/v2/config.json` 读启用 plan 的 API key/baseURL 后调 plan usage |
| CLI 指令 | P4：`codex /status` 或 TTY fallback；不优先于 OAuth/RPC | 待验证：可作登录/状态辅助，额度命令未定 | P3：`kimi` 交互输入 `/usage` | 待验证：`mmx quota` / `minimax quota` 未确认，不猜测执行 | 待验证 | 待验证：`zcode-cli` 额度命令未定 |
| HTTP API 指令 | P2：`wham/usage`，解析 primary/secondary 或 headers | P1：`api.anthropic.com` rate-limit headers | P2：Kimi coding usage；P1 可由 Web membership API 补完整额度 | P1：MiniMax coding plan remains | P1：本地 endpoint 包装成 HTTP/gRPC-Web | P1：Z Code anthropic-compatible plan endpoint |
| RPC | P1：`codex app-server` → `account/rateLimits/read` | 待验证 | 待验证 | 待验证 | P1：language_server `GetUserStatus` | 待验证 |
| Browser dashboard | P3：`chatgpt.com` cookie 兜底，避免默认扫浏览器 | P2：`claude.ai/api/organizations → usage` | P1：`www.kimi.com/...GetSubscriptionStat`，`kimi-auth` bearer | P2：`api.minimax.chat/.../coding_plan/remains`，可能受 Cloudflare 限制 | 跳过 | 跳过 |
...
| API 指令 | 跳过：`wham/usage` 主要给额度，不保证过期日 | 跳过：Anthropic usage 不等于 billing expiry | P1：`GetSubscriptionStat.subscriptionBalance.expireTime` | 跳过：`coding_plan/remains` 当前不返回过期日 | 待验证：`GetUserStatus` 可能返回 tier，不保证 expiry | 待验证：plan endpoint 是否返回 expiry |
...
| 可行方案 | Codex | Claude | Kimi | MiniMax | Antigravity | GLM / Z Code |
|---|---|---|---|---|---|---|
| API / 响应字段拿档位 | P1：`wham/usage.plan_type` → Plus/Pro | P1：Claude OAuth / account info 拿 plan，字段待稳定 | P1：`membership.level` / `GetSubscriptionStat` → Andante 等 | P1：`current_package_name` → TokenPlan/Plus/Pro 等 | P1：`GetUserStatus` → Plus/Pro/Ultra 等 | P2：plan endpoint 返回 plan id 时使用 |
| 配置文件拿档位 | P2 弱信号：`id_token` | P2：`.credentials.json` 可能有 subscriptionType | P2：OAuth/config 可辅助 | P2：config 不一定有 package，API 更准 | 待验证：本地 config | P1：`~/.zcode/v2/config.json` 的启用 plan |
