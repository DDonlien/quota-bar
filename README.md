...
- **真实额度读取**（P0）：
  ...
  - Kimi 优先读取 Kimi Desktop token-store，合并 `GetSubscriptionStat`（Work + Code + 到期日）和 `GetSubscription`（订阅名 + 价格）；CLI OAuth 仅作为 Code-only fallback，只读消费 `~/.kimi-code/credentials/kimi-code.json`、不写回（避免与真实 `kimi` CLI 竞争 refresh_token 轮换）——5 小时/周额度依赖 `kimi` CLI 自己的登录态，长期显示登录过期时需手动重新 `kimi login`
...
- **可扩展的策略链**（P1）：
  - 每个 provider 暴露一组有序 `ProviderFetchStrategy`；Codex 顺序为 OAuth → App WebView 会话 → Keychain
...
- **TCC 引导**：
  - 默认数据链路不要求任何浏览器权限，不会触发浏览器 Keychain / Full Disk Access 弹窗——2026-07-08 起彻底移除了读取 Safari/Chrome/Firefox 真实 cookie 文件的路径（此前一直被一个没有 UI 的环境变量挡着，对真实用户不可达），额度获取的登录态统一走 App 内 WebView 授权会话（`WebAuthorizationController` 一次性登录，写入 App 自己的 `WKWebsiteDataStore`，不碰系统浏览器）
- **菜单栏下拉 UI**：
  - 总费用、可用订阅计数、各服务 5 小时 / 周额度条
  - 自动 5 分钟刷新 + 手动「立即刷新」，手动刷新不会关闭 dropdown
  - 偏好设置窗口支持通用、模型、激活、日志、关于页面，可调整刷新间隔、Provider 刷新超时、语言、菜单栏图标模式和 Provider 开关（含 Antigravity）
...
当前明确维护的 Provider：Codex、Claude、Kimi、MiniMax、Antigravity、GLM / Z Code、opencode。Gemini 已从主动展示路径移除，Google 系配额优先通过 Antigravity 获取。新增 provider 不使用"其他 provider"占位，必须先明确具体名称、安装/凭证/额度/过期日来源。
...
**来源手段的四级排序**（同一信息层内按此顺序退级，`FetchPipeline.runSequential` 分层合并落地；2026-07-08 移除了原第 4 级「浏览器 Cookie 文件读取」——那条路径一直被一个没有 UI 的环境变量挡着，对真实用户不可达，功能上跟 App WebView 会话完全重叠，已删除）：
...
4. **App WebView 会话**（默认启用，最后一层）：用户在 App 内 WebView 登录一次（`WKWebsiteDataStore.default()` 持久保存），之后所有 dashboard API / headless 抓取静默复用该会话，不碰浏览器、不碰 Keychain。
...
行顺序即四级来源排序。Claude 额外多一行「本地 hook 缓存」——不是通用四级排序的一部分，是该 provider 独有的机制（见下）。
...
| 会话 API（App WebView 会话） | P2：`accounts/check` 的 `entitlement.expires_at`（JSON 稳定，替代 SPA DOM） | 跳过：无已知 billing JSON API | P1：`GetSubscription.nextBillingTime`（desktop token / WebView 会话同类同优先级；续费日 → 本地自然日减 1 = 最后有效日） | 跳过：`coding_plan/remains` 不返回日期 | 待验证：RPC `GetUserStatus` 是否带 expiry | 待验证 |
| CLI 指令 | 跳过 | 跳过：已核实 `claude auth status --json` 不返回订阅到期日（仅 email/org/subscriptionType） | 跳过：`/usage` 是额度重置，不是订阅到期 | 待验证 | 待验证 | 待验证 |
| Headless DOM（App WebView 会话，didFinish 后 settle 2s） | P3：账单页兜底（hash 路由 SPA 提取不稳定） | P1：`claude.ai/new#settings/billing` | P2：membership 页兜底 | P1：`platform.minimaxi.com/console/plan` | P1：`antigravity.google/settings`（提取成功率待验证） | 待验证：z.ai / Z Code 账号页 |
...
### opencode（不进四层矩阵——没有可用的额度接口）

opencode（`https://opencode.ai`）是一个 BYOK 聚合 CLI，本身没有稳定的额度百分比接口：
调研确认 Zen（其自家 pay-as-you-go 网关）的 credits API 返回 `Not Found`，Go（其自家订阅）的用量只能靠抓一个未公开的私有 dashboard 网页 + 浏览器 auth cookie，没有官方文档且结构可能随时变化，因此暂不采用。

当前支持仅到「已配置」这一层：读取 `~/.local/share/opencode/auth.json`（`opencode auth login` / `/connect` 写入的凭证文件），确认至少配置了一个下游 provider 即显示为可用，档位标签按配置内容显示 `Go` / `Zen` / `BYOK`，不展示额度条（对齐 Claude 的 tier-only fallback：没有真实额度数据时保持灰色，不伪造百分比）。
...
- macOS 26 (Tahoe) 或更新
- Swift 6.2 / Xcode 26+
- 不要求 Full Disk Access：额度获取不再读取任何浏览器的真实 cookie 文件（2026-07-08 移除），统一走 App 内 WebView 授权会话
...
3. Codex / Kimi / MiniMax / Z Code / opencode 使用本地配置、Desktop token 或 OAuth 路径，Antigravity 使用本地 RPC；缺失时最终会兜底到 App 内 WebView 授权会话（不读取任何浏览器的真实 cookie 文件）
...
│   │   ├── BrowserCookieReader.swift   # App WebView 会话 Cookie 读取器
...
- `MethodName` 是统一的来源分类标签（`ProviderSourceKind.checkLogLabel` / `SubscriptionExpirySourceKind.checkLogLabel`）：「配置/凭证 → API」「CLI 命令」「本地 App / RPC」「App WebView 会话」「Keychain」等，跟 README 四级来源排序用的是同一套词汇；具体是哪个 strategy（如 `kimi-desktop-token`、`claude-oauth`）放在最后的详细内容里写成"来源 `<id>`：..."，不直接拿 id 当 MethodName——id 本身猜不出属于哪一类来源。
...
Quota Bar 不需要 Full Disk Access。2026-07-08 之前有一条读取真实浏览器（Safari/Chrome/Firefox）cookie 文件的路径，被 `QUOTABAR_ENABLE_BROWSER_COOKIE` 环境变量挡着、没有任何 UI 能打开——排查后确认它对真实用户完全不可达，且功能上和 App 内 WebView 授权会话（`WebAuthorizationController`）完全重叠，已彻底删除（连同 `FilesystemCookieReader`/`EdgeCookieReader`/`SweetCookieKit` 依赖）。现在所有需要登录态的 provider 都统一走「偏好设置 → 模型」页或 dropdown 里的「打开 WebView 授权」，在 App 自己的 `WKWebsiteDataStore` 里登录一次即可，不触碰系统浏览器、不触碰 Keychain、不需要任何系统权限弹窗。
