.PHONY: help build run test app web clean deploy link all
...
help:
	...
	@echo "  make link   - 把 web/ 链接到 Vercel 项目（首次部署前跑一次）"
	@echo "  make deploy - 用 Vercel CLI 部署 web 子项目到生产环境"
...
# 把本地 web/ 链接到 Vercel 上的 quota-bar-web 项目。
# 首次跑会要求登录 + 选项目/team；之后 vercel link 会创建 .vercel/project.json
link:
	cd web && vercel link

# 用 Vercel CLI 部署 web/ 到生产环境。
