...
- 整体参考 macOS 26 系统设置与 Vibe Island 设置页：左侧 sidebar 必须使用 SwiftUI/macOS 原生 `NavigationSplitView` + `List(.sidebar)` 绘制，不手搓卡片背景；系统负责侧边栏材质、圆角、选中态和 Liquid Glass 效果。右侧 detail 独立滚动，窗口底色使用系统 `windowBackgroundColor`。
...
- 关于页 build 号读取 `CFBundleVersion`；打包脚本写入格式为 `yymmdd.hhmmss.<branchname>`，branch 名中的 `/` 统一替换为 `-`。
