...
- 计划头部订阅日期：11 pt，regular，次级灰，等宽数字；在订阅金额左侧，gap 6 pt。优先表达来源明确提供的「下次续费时间」；只提供最后有效日的 provider 保留该来源语义。Hover 显示精确时间与对应语义（`yyyy-MM-dd HH:mm:ss zzz` 本地时区，例如 `2026-06-25 22:00:00 GMT+8`），避免 `yyyy/M/d` 看不到时分的歧义；已过去的历史周期日期不得伪装成当前续费日。
- 额度标签：11 pt，medium。
...
- 菜单栏多 bar 图标中，每个可用订阅对应 1 个垂直填充 bar；外框保留细白色圆角边框，不绘制额外蓝色胶囊底。实心层固定表示最短周期额度，次级层固定表示次短周期额度，并在真实比例高度绘制清晰的横向虚线边界。高度按剩余比例映射并对齐到最近物理像素，不把 7% 人为抬高到 10%；0% 保持空值语义。相邻 bar 紧贴排列（gap 1pt），每个 bar 自身带 2pt 圆角。bar 颜色使用白色（与服务名称白色对应，菜单栏图标整体保持统一简洁）。
